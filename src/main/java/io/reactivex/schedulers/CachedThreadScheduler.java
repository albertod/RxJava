...
import io.reactivex.internal.schedulers.ScheduledAction;
import io.reactivex.internal.util.RxThreadFactory;
    ...
    private static final class EventLoopWorker extends Scheduler.Worker {
        ...
        public Disposable schedule(Action0 action, long delayTime, TimeUnit unit) {
            if (innerDisposable.isDisposed()) {
                // don't schedule, we are disposed
                return Disposables.disposed();
            }

            ScheduledAction s = threadWorker.scheduleActual(action, delayTime, unit);
            innerDisposable.add(s);
            s.addParent(innerDisposable);
            return s;
        }
    }
